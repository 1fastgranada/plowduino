...
void loop() {
  ...
  if (chuck.buttonZ && !chuck.buttonC) {
    ...
    else if (old_u_d_latch == 1 && u_d_timer.GetExpiredBy() < DEBOUNCE*4) {
      // You double-tapped the button in the right timeframe, so enter float mode
      floatDown = true;
    }
    else if (u_d_latch != 1) {
    ...
    if (u_d_latch == 2 && u_d_timer.GetExpiredBy() > DEBOUNCE) {
      ...
      // You double-tapped the button in the right timeframe, so enter autoraise mode
      autoRaise = true;
    }
    else if (u_d_latch != 2) {
